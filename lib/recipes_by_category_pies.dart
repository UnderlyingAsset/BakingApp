...
///A class which returns a list of recipes from the "pies" category within
///the Recipes collection of the database.  These recipes are shown on tiles
///which can be scrolled through and clicked on to access recipes.

class RecipeByCategoryPies extends StatefulWidget {
  const RecipeByCategoryPies({super.key});

  @override
  State<RecipeByCategoryPies> createState() => _RecipeByCategoryPiesState();
}

class _RecipeByCategoryPiesState extends State<RecipeByCategoryPies> {

  ///This will eventually be a list of documents from the database.
  List<String> docId = [];

  ///Method to collect docId for pie recipes.
  Future getDocId() async {
    docId.clear();
    await FirebaseFirestore.instance.collection('recipes')
        .where('category', isEqualTo: 'Pies').get()
