...
///A class which returns a list of recipes from the "cookies/brownies" category
...
class RecipeByCategoryCookiesBrownies extends StatefulWidget {
  const RecipeByCategoryCookiesBrownies({super.key});

  @override
  State<RecipeByCategoryCookiesBrownies> createState() => _RecipeByCategoryCookiesBrowniesState();
}

class _RecipeByCategoryCookiesBrowniesState extends State<RecipeByCategoryCookiesBrownies> {

  ///This will eventually be a list of documents from the database.
  List<String> docId = [];

  ///Method to collect docId for cookie and brownie recipes.
  Future getDocId() async {
    docId.clear();
    await FirebaseFirestore.instance.collection('recipes')
        .where('category', isEqualTo: 'Cookies/Brownies').get()
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(
          ...
          children: [
            const Text('Cookie/Brownie Recipes',
            style: TextStyle(
              fontSize: 34.0,
              fontFamily: 'Candy',
              ),
